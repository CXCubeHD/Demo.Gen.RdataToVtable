...
pub fn generate_rs(gen: &Gen) -> Result<String, Box<dyn Error>> {
    let mut ss = BufWriter::new(Vec::new());

    writeln!(ss, "#[non_exhaustive]")?;
    writeln!(ss, "#[allow(non_camel_case_types, non_snake_case)]")?;
    writeln!(ss, "#[repr(C)]")?;
    writeln!(ss, "pub struct {}_Vftable {{", gen.r#type)?;
    ...
    for (i, m) in gen.methods.iter().enumerate() {
        ...
        let line = |stream: &mut BufWriter<_>| -> Result<(), Box<dyn Error>> {
            if !is_last {
                writeln!(stream)?
            }
            Ok(())
        ...
        if m.name.trim().is_empty() {
            writeln!(ss, "\t_{}: *mut std::ffi::c_void,", m.index)?;
            line(&mut ss)?;
            continue;
        }
        ...
        if *occured_counter == 0 {
            writeln!(ss, "\tpub {}: *mut std::ffi::c_void,", m.name)?;
        } else {
            writeln!(
                ss,
                "\tpub {}_{}: *mut std::ffi::c_void,",
                m.name,
